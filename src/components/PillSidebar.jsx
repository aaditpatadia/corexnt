...
const CREATOR_NAV = [
  { id:"reel-scripts",  label:"Reel Scripts",    icon:Icons.Film       },
  { id:"growth-audit",  label:"Growth Audit",    icon:Icons.ChartUp    },
  { id:"trend-engine",  label:"Trend Engine",    icon:Icons.Fire       },
  { id:"brand-deals",   label:"Brand Deals",     icon:Icons.Handshake  },
  { id:"templates",     label:"Templates",       icon:Icons.Grid       },
  { id:"history",       label:"History",         icon:Icons.Clock      },
];
const COMPANY_NAV = [
  { id:"campaigns",     label:"Campaign Builder",icon:Icons.Megaphone  },
  { id:"budget",        label:"Budget Allocator",icon:Icons.PieChart   },
  { id:"brand-audit",   label:"Brand Audit",     icon:Icons.Shield     },
  { id:"competitor",    label:"Competitor Intel",icon:Icons.Eye        },
  { id:"reports",       label:"Reports",         icon:Icons.FileChart  },
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 py-3">
      <div className="relative flex items-center justify-center" style={{ width:52, height:52 }}>
        ...
        <span className="absolute text-xs font-bold" style={{ color:"#f0faf2", fontFamily:"Sora,sans-serif" }}>
          {total - used}
        </span>
      </div>
      <AnimatePresence>
        {expanded && (
          <motion.span initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            className="text-xs whitespace-nowrap" style={{ color:"var(--text-muted)" }}>
            messages left
          </motion.span>
        )}
      </AnimatePresence>
...
export default function PillSidebar({ userType="creator", activeTab, onTabChange, msgLeft=10, onUpgrade, userName="" }) {
        ...
        <AnimatePresence>
          {expanded && (
            <motion.div initial={{ opacity:0, x:-8 }} animate={{ opacity:1, x:0 }} exit={{ opacity:0, x:-8 }} transition={{ duration:0.18 }}
              ...
              <div className="text-sm font-bold leading-none" style={{ fontFamily:"Sora,sans-serif", color:"#f0faf2" }}>Corex</div>
              {userName && <div className="text-xs mt-0.5 truncate max-w-[130px]" style={{ color:"var(--text-muted)" }}>{userName}</div>}
            </motion.div>
          )}
        </AnimatePresence>
      </div>
